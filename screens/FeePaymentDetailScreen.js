...
const FeePaymentDetailScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (route.params?.feeData) {
      const data = route.params.feeData;
      setFeeData(data);
      setMonth(data.month || '');
  ...
  const handleCancel = () => {
    ...
    if (feeData) {
      ...
      setDuedate(feeData.duedate || '');
      setPaiddate(feeData.paiddate || '');
    }
  };
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const updatedData = { feeid: feeData.feeid, month, amount, status, duedate, paiddate };
      const response = await axios.put(`http://localhost:5000/api/fees/${feeData.feeid}`, updatedData);
  ...
  const handleDelete = () => {
    Alert.alert('Delete', 'Sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        setLoading(true);
        try {
          await axios.delete(`http://localhost:5000/api/fees/${feeData.feeid}`);
        ...
        <Card.Content>
          ...
          <View style={styles.formContainer}>
            <View style={styles.fieldGroup}>
              ...
              <TextInput style={styles.input} value={month} onChangeText={setMonth} editable={isEditMode} mode="outlined" />
            </View>
            <View style={styles.fieldGroup}>
              <Text style={styles.label}>Amount</Text>
              <TextInput style={styles.input} value={amount} onChangeText={setAmount} editable={isEditMode} mode="outlined" keyboardType="numeric" />
            </View>
            <View style={styles.fieldGroup}>
            ...
            <View style={styles.fieldGroup}>
              ...
              <TextInput style={styles.input} value={duedate} onChangeText={setDuedate} editable={isEditMode} mode="outlined" />
            </View>
            <View style={styles.fieldGroup}>
              <Text style={styles.label}>Paid Date</Text>
              <TextInput style={styles.input} value={paiddate} onChangeText={setPaiddate} editable={isEditMode} mode="outlined" />
            </View>
...
export default FeePaymentDetailScreen;
