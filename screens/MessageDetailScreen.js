...
const MessageDetailScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (route.params?.messageData) {
      const data = route.params.messageData;
      setMessageData(data);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const updatedData = { messageid: messageData.messageid, sender, subject, body, date };
      const response = await axios.put(`http://localhost:5000/api/messages/${messageData.messageid}`, updatedData);
  ...
  const handleDelete = () => {
    Alert.alert('Delete', 'Sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        setLoading(true);
        try {
          await axios.delete(`http://localhost:5000/api/messages/${messageData.messageid}`);
        ...
        <Card.Content>
          ...
          <View style={styles.formContainer}>
            <View style={styles.fieldGroup}>
              ...
              <TextInput style={styles.input} value={sender} onChangeText={setSender} editable={false} mode="outlined" />
            </View>
            <View style={styles.fieldGroup}>
            ...
            <View style={styles.fieldGroup}>
              ...
              <TextInput style={[styles.input, { height: 120 }]} value={body} onChangeText={setBody} editable={isEditMode} mode="outlined" multiline />
            </View>
            <View style={styles.fieldGroup}>
              <Text style={styles.label}>Date</Text>
              <TextInput style={styles.input} value={date} onChangeText={setDate} editable={false} mode="outlined" />
            </View>
